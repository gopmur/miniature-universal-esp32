...
void dns_question_byte_inverse(DNSPacket* packet) {
  ...
  packet->question.clss = BYTE_INVERSE_16(packet->question.clss);
}

void dns_answer_byte_inverse(DNSPacket* packet) {
  packet->answer.clss = BYTE_INVERSE_16(packet->answer.clss);
  ...
  while (true) {
    ...
    while (i < bytes_received) {
      if (dns_parser_state == DNS_PARSER_STATE_HEADER) {
        ...
        i += label_len;
      } else if (dns_parser_state == DNS_PARSER_STATE_TYPE_clss) {
